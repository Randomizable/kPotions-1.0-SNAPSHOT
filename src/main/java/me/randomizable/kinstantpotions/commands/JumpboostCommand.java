...
public class JumpboostCommand implements CommandExecutor, Listener {

    private kPotions plugin;

    public JumpboostCommand(kPotions plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (sender instanceof Player) {
            ...
            if (kPotions.getInstance().getConfig().getBoolean("enable_jumpboost_command")) {
                if (player.hasPermission("kpotions.jumpboost")){
                    if (!player.isDead() && !player.hasPotionEffect(PotionEffectType.JUMP)) {
                        player.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getInstance().getConfig().getString("jumpboost_enabled")));
                        player.addPotionEffect(new PotionEffect(PotionEffectType.JUMP, 999999999, 1));
                    }else{
                        player.sendMessage(ChatColor.translateAlternateColorCodes('&', kPotions.getInstance().getConfig().getString("jumpboost_disabled")));
